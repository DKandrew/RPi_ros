...
void config_imu(int channel){
	...
	unsigned char data[dataLen] = {0x03, 0x00, 0x00, 0x00, 0x30, 0x00, 0xD1};
	wiringPiSPIDataRW(channel, data, dataLen);
}
	...
	*/
	...
	unsigned char data1[11] = {0x03, 0x00, 0x00, 0x00,
     ...
     unsigned char data3[11] = {0x03, 0x00, 0x00, 0x00,
							 ...
							 0x03, 0xE8, 0xD2};
   int dataLen = 11;
   wiringPiSPIDataRW(channel, data3, dataLen);
...
void measurement_imu(int channel){
	...
	unsigned char data[dataLen] = {0x03, 0x00, 0x00, 0x00, 0x10, 0x00, 0xF1};
	wiringPiSPIDataRW(channel, data, dataLen); 
}
...
float* readMeasurement_imu(int channel,int dataLen){
	...
	float wz = *((float*)&w3);
	  ...
	  float result2[9] = {x,y,z,ax,ay,az,wx,wy,wz};
...
float* readMeasurement_imu_acc(int channel,int dataLen){
	...
	//float wy = *((float*)&w2);
	//float wz = *((float*)&w3);
	//float result1[6] = {ax,ay,az,wx,wy,wz};
	float result1[3] = {ax,ay,az};
	return result1;
}
...
int main(int argc, char** argv){
	...
	*/
	...
	while(ros::ok()&& (i < readlength)){  // When wirting data into file, add this into the while codition ->  	&& (i < readlength)	
		//result = readMeasurement_imu_acc(channel_imu,25);
		result = readMeasurement_imu_acc(channel_imu,25);
	/*
		float x, az, wx, wy, wz;
